package app.interview.ale.beer.data.local.datastore
...
class DataStoreManager @Inject constructor(@ApplicationContext appContext: Context) {
    ...
    suspend fun setThemeMode(mode: Int) = settingsDataStore.edit { settings -> settings[app.interview.ale.beer.base.preference.AppSettings.NIGHT_MODE] = mode }

    suspend fun <T> getSetting(key: Preferences.Key<T>) = settingsDataStore.data.map { it[key] }.firstOrNull().also { Timber.d("Get setting ${key.name} : $it") }
